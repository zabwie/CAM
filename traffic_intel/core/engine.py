...
class TrafficEngine:
    ...
    def __init__(
        ...
        # Compatibility attributes used by older scripts.
        self.conf_thresh = tracking.detector_confidence
        self.track_activation_threshold = tracking.activation_threshold
        self.track_min_hits = tracking.min_hits
        self.track_min_confidence = tracking.min_confidence
        self.vehicle_nms_threshold = tracking.vehicle_nms_threshold
        self.scene_cut_reset = scene.enabled
    ...
    def results_csv(self, path: PathLike) -> None:
    ...
    def _draw_vehicle_label(
        ...
        *,
        ...
        """Backward-compatible label renderer used by ``replay.py``."""
        speed_text = f"{speed_mph:.0f} MPH" if speed_mph is not None else "-- MPH"
        quality = f" | Q{confidence * 100:.0f}%" if confidence is not None else ""
        text = f"{class_name} | {speed_text}{quality} | #{track_id}"
        font = cv2.FONT_HERSHEY_SIMPLEX
        scale, thickness = 0.52, 2
        (text_w, text_h), baseline = cv2.getTextSize(text, font, scale, thickness)
        frame_h, frame_w = frame.shape[:2]
    ...
    def _detect_scene_cut(self, frame: np.ndarray) -> bool:
        # Compatibility fallback for tests/legacy instances created with __new__.
        detector = getattr(self, "scene_detector", None)
        if detector is not None:
            return detector.update(frame)

        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        small = cv2.resize(gray, (160, 90), interpolation=cv2.INTER_AREA)
        previous = getattr(self, "_prev_scene_gray", None)
        self._prev_scene_gray = small
        if previous is None or previous.shape != small.shape:
            return False
        diff = cv2.absdiff(previous, small)
        return bool(float(diff.mean()) > 15.0 and float(np.mean(diff > 32)) > 0.15)
