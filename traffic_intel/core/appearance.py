...
def appearance_descriptor(
    ...
    # True IR night feeds are monochrome — no colour identity signal.
    channel_delta = np.max(crop, axis=2).astype(np.float32) - np.min(crop, axis=2).astype(np.float32)
    if float(np.median(hsv[..., 1])) < 10.0 and float(np.mean(channel_delta)) < 4.0:
        return None
