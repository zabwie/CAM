...
def main() -> None:
    ...
    missing_videos = [
        ...
        if not (ROOT / "validation" / "videos" / f"{name}.mp4").exists()
    ]
    if missing_videos:
        missing = "\n".join(f"  - {path}" for path in missing_videos)
        raise SystemExit(
            "Source validation videos are not present in this slim package.\n"
            "The cached detector JSON is not a drop-in substitute because source pixels "
            "are used for identity appearance matching and scene resets.\n"
            f"Missing:\n{missing}\n"
            "Run `python tools/stress_crash_detector.py` for the self-contained adversarial suite, "
            "or restore the original validation/videos clips for the full cached replay."
        )
    output = {}
    failures = []
    for name in ("crash", "crash2"):
